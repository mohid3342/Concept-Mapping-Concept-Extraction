...
# Use GENERAL anchors (safe default)
CS_ANCHORS = [
...
anchor_embeddings = encoder_model.encode(CS_ANCHORS)
...
def encode_lecture(lecture_text: str) -> str:
    """
    Semantic encoder: converts raw slides into structured + scored format
    """
    ...
    for i, slide in enumerate(slides):
        ...
        # Batch encode (IMPORTANT: faster)
        line_embeddings = encoder_model.encode(lines)
        '''
        for line, vec in zip(lines, line_embeddings):
            sims = [cosine_sim(vec, anchor) for anchor in anchor_embeddings]
            score = max(sims)
            ...
            if score >= 0.6:
                ...
                tag = "LOW"
            print(f"Line: {line[:50]}... | Score: {score:.4f} | Tag: {tag} | Sim: {sims} | Embedding norm: {np.linalg.norm(vec):.4f}")
            out.append(f"[{tag} | score={score:.2f}] {line}")
        '''
        for line, vec in zip(lines, line_embeddings):
            #print(f"Line: {line[:50]}... | Embedding norm: {np.linalg.norm(vec):.4f}| Embedding: {vec}... | Line embedding {line_embeddings}")
            print(f"Vec for line {line[:50]}...: {vec}")
            out.append(vec)
...
def format_example(example):
    lecture_json = encode_lecture(example["input"])
...
sft_config = SFTConfig(
    output_dir="./qwen2-finetuned3_cs007_encoded",
...
trainer.train()
trainer.save_model("./qwen2-finetuned3_cs007_encoded")
...
base_model,
"./qwen2-finetuned3_cs007_encoded"
)
...
with open('evaluation_results_cs0007_encoded.json', 'w') as f:
    json.dump(results, f, indent=2)

print("\n" + "="*60)
print("Results saved to 'evaluation_results_cs0007_format.json'")
print("="*60)
