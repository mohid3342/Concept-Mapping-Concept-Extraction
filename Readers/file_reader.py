...
class PowerPointToJSONConverter:
    ...
    def extract_pptx_data(self, file_path):
        ...
        for slide_index, slide in enumerate(prs.slides, start=1):
            ...
            for shape in slide.shapes:
                ...
                if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
                    alt_text = self.get_alt_text(shape)
                    try:
                        image_base64 = self.image_to_base64(shape.image)
                        ai_description = self.describe_image_with_ai(image_base64)
    ...
    def convert_pptx_to_json(self, pptx_path, output_path):
        ppt_structure = self.extract_pptx_data(pptx_path)
